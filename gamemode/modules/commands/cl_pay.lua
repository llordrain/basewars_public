...
function PANEL:Init()
    ...
    self.Title.Paint = function(s,w,h)
        BaseWars:DrawRoundedBoxEx(6, 0, 0, w, h, GetBaseWarsTheme("pay_titleBar"), true, true)
        draw.SimpleText(string.upper(ply:GetLang("pay_title")), "BaseWars.24", w * .5, h * .5, GetBaseWarsTheme("pay_text"), 1, 1)
    end
    ...
    self.MiddlePanel.player.Draw = function(s,w,h)
        draw.SimpleText(ply:GetLang("choosePlayer"), "BaseWars.20", bigMargin, h * .5, GetBaseWarsTheme("pay_text"), 0, 1)
        draw.SimpleText(self.selectedPlayer:Name(), "BaseWars.20", w - bigMargin, h * .5, GetBaseWarsTheme("pay_text"), 2, 1)
    end
    ...
    self.MiddlePanel.Amount:SetColor(GetBaseWarsTheme("pay_contentBackground"))
    self.MiddlePanel.Amount:SetTextColor(GetBaseWarsTheme("pay_text"))
    self.MiddlePanel.Amount:SetFont("BaseWars.18")
    self.MiddlePanel.Amount:SetPlaceHolder(ply:GetLang("amount"))
    self.MiddlePanel.Amount:SetPlaceHolderColor(GetBaseWarsTheme("pay_darkText"))
    ...
    self.BottomPanel.Paint = function(s,w,h)
        BaseWars:DrawRoundedBox(4, 0, 0, w, h, GetBaseWarsTheme("pay_contentBackground"))
    end
    ...
    self.Pay.Draw = function(s,w,h)
        ...
        draw.SimpleText(LocalPlayer():GetLang("pay_giveTo"):format(BaseWars:FormatMoney(amount), self.selectedPlayer:Name()), "BaseWars.18", w * .5, h * .5, GetBaseWarsTheme("pay_text"), 1, 1)
    end
    ...
    self.Close.Draw = function(s,w,h)
        BaseWars:DrawMaterial(closeIcon, (w - h * .5) * .5, bigMargin, h * .5, h * .5, GetBaseWarsTheme("pay_text"))
    end
