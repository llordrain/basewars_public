...
function PANEL:Init()
	...
	if BaseWars:IsSuperAdmin(self.localPlayer) then
		...
		self.Topbar.PlayerConfig.Draw = function(s,w,h)
			draw.SimpleText(self.localPlayer:GetLang("config_playerConfig"), "BaseWars.24", w * .5, h * .5, BaseWars:GetTheme("bwm_text"), 1, 1)
		end
		...
		self.Topbar.GamemodeConfig.Draw = function(s,w,h)
			draw.SimpleText(self.localPlayer:GetLang("config_gamemodeConfig"), "BaseWars.24", w * .5, h * .5, BaseWars:GetTheme("bwm_text"), 1, 1)
		end
...
function PANEL:Build(config, filter)
	...
	if config == "gamemode" and BaseWars:IsSuperAdmin(self.localPlayer) then
		...
		if not IsValid(self.Top) then
			...
			self.Top.Filter:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
			self.Top.Filter:SetTextColor(BaseWars:GetTheme("bwm_text"))
			self.Top.Filter:SetFont("BaseWars.18")
			self.Top.Filter:SetPlaceHolder(self.localPlayer:GetLang("search"))
			self.Top.Filter:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
		...
		if not IsValid(self.Bottom) then
			...
			self.SaveConfig.Draw = function(s,w,h)
				draw.SimpleText(self.localPlayer:GetLang("gamemodeConfig_saveConfig"), "BaseWars.20", w * .5, h * .5, BaseWars:GetTheme("bwm_text"), 1, 1)
			end
			...
			self.ResetConfig.Draw = function(s,w,h)
				draw.SimpleText(self.localPlayer:GetLang("gamemodeConfig_resetConfig"), "BaseWars.20", w * .5, h * .5, BaseWars:GetTheme("bwm_text"), 1, 1)
			end
		...
		for k, v in SortedPairsByMemberValue(configTyped, "type") do
			...
			configPanel.lerpColor = BaseWars:GetTheme("bwm_contentBackground")
			configPanel.Paint = function(s,w,h)
				if not isTable then
					s.lerpColor = BaseWars:LerpColor(FrameTime() * 15, s.lerpColor, (s:IsHovered() or s:IsChildHovered()) and BaseWars:GetTheme("bwm_contentBackground2") or BaseWars:GetTheme("bwm_contentBackground"))
				end

				BaseWars:DrawRoundedBox(4, 0, 0, w, h, s.lerpColor)
				draw.SimpleText(self.localPlayer:GetLang("gamemodeConfig_" .. k .. "Name"), "BaseWars.22", bigMargin, h * .5, BaseWars:GetTheme("bwm_text"), 0, TEXT_ALIGN_BOTTOM)
				draw.SimpleText(self.localPlayer:GetLang("gamemodeConfig_" .. k .. "Desc"), "BaseWars.18", bigMargin, h * .5, BaseWars:GetTheme("bwm_darkText"), 0, TEXT_ALIGN_TOP)
			end
		...
		for k, v in SortedPairsByMemberValue(BaseWars.DefaultPlayerConfig, "type") do
			...
			configPanel.Paint = function(s,w,h)
				...
				BaseWars:DrawRoundedBox(4, 0, 0, w, h, s.lerpColor)
				draw.SimpleText(self.localPlayer:GetLang("f3config", k .. "Name"), "BaseWars.22", bigMargin, h * .5, BaseWars:GetTheme("bwm_text"), 0, TEXT_ALIGN_BOTTOM)
				draw.SimpleText(self.localPlayer:GetLang("f3config", k .. "Desc"):format(input.LookupBinding("gm_showspare2") or "?"), "BaseWars.18", bigMargin, h * .5, BaseWars:GetTheme("bwm_darkText"), 0, TEXT_ALIGN_TOP)
			end
			...
			if v.type == "boolean" then
				...
				configPanel.Action.OnToggle = function(s, state)
					BaseWars:SetPlayerConfig(k, state)
				end
			end
			...
			if v.choices then
				...
				configPanel.Action.Draw = function(s,w,h)
					local choice = v.choices[s.value]

					if choice[1] == "#" then
						choice = self.localPlayer:GetLang(string.sub(choice, 2))
					end

					draw.SimpleText(choice or "???", "BaseWars.20", w * .5, h * .5, BaseWars:GetTheme("bwm_text"), 1, 1)
				end
				configPanel.Action.DoClick = function(s)
					...
					for id, name in SortedPairs(v.choices) do
						...
						if name[1] == "#" then
							name = self.localPlayer:GetLang(string.sub(name, 2))
						end

						dropDown:AddChoice(name, function()
							BaseWars:SetPlayerConfig(k, id)
							s.value = id
						end)
					end
				end
...
function PANEL:BuildTable(key, parent, subParent)
	...
	if key == "PrinterCap" then
		...
		for k, v in SortedPairs(tbl) do
			...
			item.lerpColor = BaseWars:GetTheme("bwm_contentBackground")
			item.Paint = function(s,w,h)
				s.lerpColor = BaseWars:LerpColor(FrameTime() * 15, s.lerpColor, (s:IsHovered() or s:IsChildHovered()) and BaseWars:GetTheme("bwm_contentBackground2") or BaseWars:GetTheme("bwm_contentBackground"))

				BaseWars:DrawRoundedBox(4, 0, 0, w, h, s.lerpColor)
				draw.SimpleText(k, "BaseWars.22", bigMargin, h * .5, BaseWars:GetTheme("bwm_text"), 0, TEXT_ALIGN_CENTER)
			end
			...
			item.RemoveItem:SetColor(BaseWars:GetTheme("bwm_contentBackground"), true)
			item.RemoveItem.Draw = function(s,w,h)
				BaseWars:DrawMaterial(trash, w * .5, h * .5, h - bigMargin * 1.5, h - bigMargin * 1.5, BaseWars:GetTheme("bwm_text"), 0)
			end
		...
		parent.Action:SetColor(BaseWars:GetTheme("bwm_contentBackground"), true)
		parent.Action.Draw = function(s,w,h)
			BaseWars:DrawMaterial(plus, w * .5, h * .5, h - bigMargin * 1.5, h - bigMargin * 1.5, BaseWars:GetTheme("bwm_text"), 0)
		end
		...
		parent.Number:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.Number:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.Number:SetFont("BaseWars.18")
		parent.Number:SetPlaceHolder(self.localPlayer:GetLang("config_number"))
		parent.Number:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
		...
		parent.UserGroup:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.UserGroup:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.UserGroup:SetFont("BaseWars.18")
		parent.UserGroup:SetPlaceHolder(self.localPlayer:GetLang("gamemodeConfig_userGroup"))
		parent.UserGroup:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
	end
	...
	if BaseWars.DefaultConfigType[key] == BWCONFIGTYPE_KEYVALUE then
		...
		for k, v in SortedPairs(tbl) do
			...
			item.Paint = function(s,w,h)
				...
				BaseWars:DrawRoundedBox(4, 0, 0, w, h, s.lerpColor)
				draw.SimpleText(self.localPlayer:GetLang("gamemodeConfig_" .. key, k .. "Name"), "BaseWars.22", bigMargin, h * .5, BaseWars:GetTheme("bwm_text"), 0, TEXT_ALIGN_BOTTOM)
				draw.SimpleText(self.localPlayer:GetLang("gamemodeConfig_" .. key, k .. "Desc"), "BaseWars.18", bigMargin, h * .5, BaseWars:GetTheme("bwm_darkText"), 0, TEXT_ALIGN_TOP)
			end
	...
	if key == "FormatNumber" then
		...
		for k, v in SortedPairs(tbl) do
			...
			item.Paint = function(s,w,h)
				...
				BaseWars:DrawRoundedBox(4, 0, 0, w, h, s.lerpColor)
				draw.SimpleText(v[2] .. " [" .. v[3] .. "]", "BaseWars.20", bigMargin, h * .5, BaseWars:GetTheme("bwm_text"), 0, 1)
			end
		...
		parent.ShortText:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.ShortText:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.ShortText:SetFont("BaseWars.18")
		parent.ShortText:SetPlaceHolder(self.localPlayer:GetLang("gamemodeConfig_shortText"))
		parent.ShortText:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
		...
		parent.Text:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.Text:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.Text:SetFont("BaseWars.18")
		parent.Text:SetPlaceHolder(self.localPlayer:GetLang("config_string"))
		parent.Text:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
	...
	if BaseWars.DefaultConfigType[key] == BWCONFIGTYPE_NOKEY then
		...
		if key == "WhitelistProps" then
			...
			for k, v in ipairs(tbl) do
				...
				item.Paint = function(s,w,h)
					...
					BaseWars:DrawRoundedBox(4, 0, 0, w, h, s.lerpColor)
					draw.SimpleText(text, "BaseWars.20", bigMargin, h * .5, BaseWars:GetTheme("bwm_text"), 0, 1)
				end
		...
		parent.TextEntry:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.TextEntry:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.TextEntry:SetFont("BaseWars.18")
		parent.TextEntry:SetPlaceHolder(self.localPlayer:GetLang("gamemodeConfig_" .. placeHolder))
		parent.TextEntry:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
	end
	...
	if BaseWars.DefaultConfigType[key] == BWCONFIGTYPE_KEY then
		...
		for k, v in SortedPairs(tbl) do
			...
			item.Paint = function(s,w,h)
				...
				BaseWars:DrawRoundedBox(4, 0, 0, w, h, s.lerpColor)
				draw.SimpleText(k, "BaseWars.20", bigMargin, h * .5, BaseWars:GetTheme("bwm_text"), 0, 1)
			end
...
function PANEL:BuildAction(type, parent, defaultValue, tbl, key)
	...
	if type == "number" then
		...
		parent.Action:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.Action:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.Action:SetFont("BaseWars.18")
		parent.Action:SetPlaceHolder(self.localPlayer:GetLang("config_number"))
		parent.Action:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
	...
	if type == "Vector" then
		...
		parent.VectorZ:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.VectorZ:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.VectorZ:SetFont("BaseWars.18")
		parent.VectorZ:SetPlaceHolder(self.localPlayer:GetLang("config_vector_z"))
		parent.VectorZ:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
		...
		parent.VectorY:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.VectorY:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.VectorY:SetFont("BaseWars.18")
		parent.VectorY:SetPlaceHolder(self.localPlayer:GetLang("config_vector_y"))
		parent.VectorY:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
		...
		parent.VectorX:SetColor(BaseWars:GetTheme("bwm_contentBackground2"))
		parent.VectorX:SetTextColor(BaseWars:GetTheme("bwm_text"))
		parent.VectorX:SetFont("BaseWars.18")
		parent.VectorX:SetPlaceHolder(self.localPlayer:GetLang("config_vector_x"))
		parent.VectorX:SetPlaceHolderColor(BaseWars:GetTheme("bwm_darkText"))
