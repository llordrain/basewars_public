...
function PANEL:Init()
	...
	self.SidePanel.Infos.Paint = function(s,w,h)
		...
		local x, y = w * .5, w * .5
		BaseWars:DrawCircle(x, y, w * .35, 360, 0, 360, GetBaseWarsTheme("bwm_contentBackground2"))
		BaseWars:DrawCircle(x, y, w * .35, 360, 0, 360 * math.Clamp(percent, 0, 1), GetBaseWarsTheme("gen_accent"))
		BaseWars:DrawCircle(x, y, w * .32, 360, 0, 360, GetBaseWarsTheme("bwm_contentBackground"))

		draw.SimpleText(string.Comma(math.Round(percent * 100, 2)) .. "%", "BaseWars.30", w * .5, y, GetBaseWarsTheme("bwm_text"), 1, TEXT_ALIGN_BOTTOM)
		draw.SimpleText(BaseWars:FormatNumber(s.plyLevel, true) .. "/" .. BaseWars:FormatNumber(levelForPrestige, true), "BaseWars.20", w * .5, y, GetBaseWarsTheme("bwm_darkText"), 1, TEXT_ALIGN_TOP)
	end
	...
	self.SidePanel.DoPrestige.Button.Draw = function(s,w,h)
		draw.SimpleText(ply:GetLang("prestige_doPrestige"):format(BaseWars:FormatNumber(ply:GetPrestige() + 1)), "BaseWars.20", w * .5, h * .5, GetBaseWarsTheme("bwm_text"), 1, 1)
	end
	...
	self.SidePanel.ResetPrestigePoint.Button.Draw = function(s,w,h)
		draw.SimpleText(ply:GetLang("prestige_resetPoint"):format(BaseWars:FormatMoney(BaseWars.Config.Prestige.ResetPrice * ply:GetPrestigePointSpent(), true)), "BaseWars.20", w * .5, h * .5, GetBaseWarsTheme("bwm_text"), 1, 1)
	end
	...
	self.SidePanel.PrestigePoint.Paint = function(s,w,h)
		BaseWars:DrawRoundedBox(4, 0, 0, w, h, GetBaseWarsTheme("bwm_contentBackground"))
		draw.SimpleText(ply:GetLang("prestige_prestigePoint"):format(BaseWars:FormatNumber(ply:GetPrestigePoint())), "BaseWars.18", w * .5, h * .5, GetBaseWarsTheme("bwm_text"), 1, 1)
	end
	...
	for k, v in SortedPairsByMemberValue(BaseWars:GetPrestigePerk(), "cost") do
		...
		perk.Paint = function(s,w,h)
			...
			draw.SimpleText(ply:GetLang("prestige_perks", k .. "Name"), "BaseWars.22", bigMargin, h * .5, GetBaseWarsTheme("bwm_text"), 0, TEXT_ALIGN_BOTTOM)
			draw.SimpleText(ply:GetLang("prestige_perks", k .. "Desc"), "BaseWars.18", bigMargin, h * .5, GetBaseWarsTheme("bwm_darkText"), 0, TEXT_ALIGN_TOP)

			draw.SimpleText(ply:GetLang("prestige_price"), "BaseWars.18", w - BaseWars.ScreenScale * 90, h * .5, GetBaseWarsTheme("bwm_text"), TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM)
			draw.SimpleText(v.cost, "BaseWars.18", w - BaseWars.ScreenScale * 85, h * .5, GetBaseWarsTheme("bwm_darkText"), 0, TEXT_ALIGN_BOTTOM)

			draw.SimpleText(ply:GetLang("prestige_level"), "BaseWars.18", w - BaseWars.ScreenScale * 90, h * .5, GetBaseWarsTheme("bwm_text"), TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
			draw.SimpleText(ply:GetPrestigePerk(k) .. "/" .. v.max, "BaseWars.18", w - BaseWars.ScreenScale * 85, h * .5, GetBaseWarsTheme("bwm_darkText"), 0, TEXT_ALIGN_TOP)
		end
		...
		perk.Buy.color = GetBaseWarsTheme("bwm_darkText")
